...
class AudioTextContrastiveLoss(nn.Module):
    ...
    def forward(self, audio_embed, text_embed):
        ...
        loss_a2t = -torch.sum(F.log_softmax(sim_a2t, dim=1) * sim_targets,
                              dim=1).mean()

        loss_t2a = -torch.sum(F.log_softmax(sim_t2a, dim=1) * sim_targets,
                              dim=1).mean()

        loss_atc = (loss_a2t + loss_t2a) / 2
        if self.embed_regularization:
            loss_atc = loss_atc + torch.mean(torch.abs(audio_embed)) / torch.sqrt(torch.sum(audio_embed**2)) + \
                torch.mean(torch.abs(text_embed)) / \
                torch.sqrt(torch.sum(text_embed**2))
        return loss_atc


class RetrivalMLP(nn.Module):

    def __init__(self, in_features, hidden_features:int = 1024, out_features: int | None = None, criterion="AudioTextContrastiveLoss"):
        super().__init__()
        out_features = out_features or in_features
        from transformers import AutoTokenizer
        self.ln = nn.LayerNorm(in_features)
        self.audio_mlp = nn.Sequential(nn.Linear(in_features, hidden_features),
                                       nn.GELU(),
                                       nn.Linear(hidden_features, out_features))
        self.text_embedding = AutoTokenizer.from_pretrained('bert-base-uncased')
        self.text_mlp = nn.Sequential(nn.Linear(text_dim, hidden_features),
                                       nn.GELU(),
                                       nn.Linear(hidden_features, out_features))
